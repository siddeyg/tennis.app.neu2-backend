...
router.get('/',
  ...
  try {
    ...
    const formattedUsers = portalUsers.map(user => ({
      ...
      familyMembersCount: Array.isArray(user.familyMembers) ? user.familyMembers.length : 0,
      familyMembers: (user.familyMembers || []).map(f => `${f.firstName || ''} ${f.lastName || ''} (${f.relationship || 'Familie'})`.trim()).filter(Boolean).join('; '),
